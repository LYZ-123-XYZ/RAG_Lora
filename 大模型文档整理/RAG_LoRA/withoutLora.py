...
zh_en_model_path = './opus-mt-zh-en/'
zh_en_tokenizer = AutoTokenizer.from_pretrained(zh_en_model_path)
zh_en_model = AutoModelForSeq2SeqLM.from_pretrained(zh_en_model_path)

en_zh_model_path = './opus-mt-en-zh/'
en_zh_tokenizer = AutoTokenizer.from_pretrained(en_zh_model_path)
en_zh_model = AutoModelForSeq2SeqLM.from_pretrained(en_zh_model_path)
...
def convert_links_to_html(text):
    ...
    return re.sub(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', replace_link, text)


def process_question(question, similarity_top_k):
    ...
    source1 = scored_data[0][1][:150]
    source2 = scored_data[1][1][:150]
    ...
    # 计算可用于添加来源文本的剩余长度
    max_prompt_length = 512 - len(local_tokenizer.encode(query, return_tensors='pt')[0]) - len(
        local_tokenizer.encode("Please provide an answer based solely on the provided sources.",
                                return_tensors='pt')[0]) - 20  # 预留一些额外字符用于标点等

    # 确保来源文本不超过剩余长度
    source1_truncated = source1[:max_prompt_length // 2]
    source2_truncated = source2[:max_prompt_length // 2]

    start_time = time.time()
    prompt = "Please provide an answer based solely on the the provided sources. Source 1:" + source1_truncated + ",Source 2:" + source2_truncated
    local_messages = [
        {"role": "System", "content": prompt},
        {"role": "User", "content": query}
    ]
    local_text = local_tokenizer.apply_chat_template(local_messages, tokenize=False, add_generation_prompt=True)
    local_model_inputs = local_tokenizer([local_text], return_tensors="pt").to(device)
    ...
    model = en_zh_model
    input_ids = tokenizer.encode(local_response, return_tensors='pt')
    outputs = model.generate(input_ids)
    response1_content = tokenizer.decode(outputs[0], snap_shot=False, skip_special_tokens=True)
    end_time = time.time()
